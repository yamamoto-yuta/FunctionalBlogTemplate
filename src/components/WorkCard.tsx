...
const WorksCard = ({ workData }: { workData: WorkData }) => {
  return (
    <Card sx={{ width: {xs: '100%', sm: 250}, height: {xs: 400, sm: 450}, mb: '1rem', overflowY: 'auto' }}>
      <Grid container sx={{mt: 1, mb: 1}}>
        <Grid item xs={4} sm={3}/>
        <Grid item xs={4} sm={6}>
          <Button href={workData.url} target="_blank" rel="noopener" sx={{width: '100%'}}>
            <CardMedia 
              component='img' 
      ...
      <CardContent sx={{pt: 0, pl: '1rem', pr: '1rem', pb: 1}}>
        <Typography gutterBottom fontSize={22} fontWeight='medium' align='center' component="div">
          <Link
              variant="h3"
              href={workData.url}
              target="_blank"
...
export const WorksCardCarousel = ({ workDataList }: { workDataList: WorkData[] }) => {
    return (
        <Swiper
            slidesPerView={1}
            pagination={{
